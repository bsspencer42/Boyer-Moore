...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern, CharSequence text, CharacterComparator comparator) {
        ...
        for (Character key : lastTable.keySet()){
            System.out.println(key + " : " + lastTable.get(key));
        }
        // Return list initilization
        List<Integer> matches = new ArrayList<>();
        // Begin compare
        // Pointer to pattern index
        int i = 0;
        int j;
        int shift;
        // Iterate through text until end remainder of text is shorter than pattern
        while (i <= text.length()-pattern.length()){
            // Reset j pointer
            j = pattern.length()-1;
            // While text index matches pattern index, decrement text index
            while (j >= 0 && comparator.compare(text.charAt(i+j),pattern.charAt(j)) == 0){
                // Decrement j
                j--;
            }
            // Match found (indexed beyond end of pattern
            if (j == -1){
                matches.add(i);
                i++;
            }
            else {
                // Get last occurrence from table
    ...
    public static void main(String[] args) {
        CharSequence pattern = "pot";
        CharSequence text = "potatopoto";
        CharacterComparator comparator = new CharacterComparator();
        List<Integer> myList = boyerMoore(pattern,text,comparator);
